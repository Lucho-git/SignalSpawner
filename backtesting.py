...
def get_sample_long_data():
    return ['MAGICUSDT', client.KLINE_INTERVAL_5MINUTE, '1684059363825', 0.837, [0.84043,0.85021,0.85941,0.90288], 0.7661]

def get_sample_short_data():
    return ['ETHUSDT', client.KLINE_INTERVAL_5MINUTE, '1683970193138', 1830, [1820.85, 1811.7, 1793.4, 1740.33, 1692.0], 1955.2]
...
def calculate_profit_percentage(entry_price, target_price, direction):
    if direction == 'LONG':
...
def backtest(data, entry_price, take_profit_values, stop_loss, start_time, direction):
    ...
    if direction == 'LONG':
        ...
        take_profit_values.sort(reverse=True)
    ...
    for index, row in data.iterrows():
        ...
        if not entered:
            continue
        if (direction == 'LONG' and row['low'] <= stop_loss) or (direction == 'SHORT' and row['high'] >= stop_loss):
            stop_loss_percentage = calculate_profit_percentage(entry_price, stop_loss, direction)
        ...
        for take_profit in take_profit_values:
            if (direction == 'LONG' and row['high'] >= take_profit) or (direction == 'SHORT' and row['low'] <= take_profit):
                take_profit_hits += 1
                take_profit_times[take_profit] = round((row['open_time'] - start_time) / (1000 * 60 * 60), 2)  # Time difference in hours
                take_profit_percentages[take_profit] = calculate_profit_percentage(entry_price, take_profit, direction)
...
def run_backtest():
    ...
    result_type, result_price, result_time, take_profit_hits, take_profit_times, stop_loss_percentage, take_profit_percentages = backtest(kline_data, backtesting_data[3], backtesting_data[4], backtesting_data[5], backtesting_data[2], 'LONG')
